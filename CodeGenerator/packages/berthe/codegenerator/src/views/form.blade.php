...
@section('title'){{ ucfirst($table['title'])}} @endsection

@section('content')<form action="/{{$table['title']}}" method="post">@if( array_key_exists('attributs', $table) )@foreach($table['attributs'] as $attrName => $attrType)
		...
		@if(gettype($attrType) == gettype(0))    	<div class="form-group">
						<label id="{{$attrName}}">{{ucfirst($attrName)}}</label>
